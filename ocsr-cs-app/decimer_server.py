...
from rdkit.Chem import MolToMolBlock, MolFromSmiles, MolFromMolBlock
...
def service_connection(key, mask):
  ...
  if mask & selectors.EVENT_READ:
    ...
    if recv_data:
      smiles  = predict_SMILES(input_img(recv_data))
      try:
        molfile = MolToMolBlock(MolFromSmiles(smiles))
        data.outb = str({
          "smiles": smiles,
          "molfile": molfile,
        }).encode()
        mol = MolFromMolBlock(molfile)
...
def main():
  """主函数，启动 next 服务器"""
  host, port = "0.0.0.0", ports['decimer']
